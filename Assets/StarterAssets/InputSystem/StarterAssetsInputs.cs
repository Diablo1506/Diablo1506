...
namespace StarterAssets
{
    public class StarterAssetsInputs : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) && Input.GetKey(KeyCode.C)) // 0 is the left mouse button
            {
                Debug.Log($"#{GetType()}:  jabbing left");
                Get.GameManager.PlayerController.OnPunch(PunchID.LEFTJAB);
            }
            else
            {
            ...
            if (Input.GetMouseButtonDown(1) && Input.GetKey(KeyCode.C)) // 1 is the right mouse button
            {
                Debug.Log($"#{GetType()}:  jabbing right");
                Get.GameManager.PlayerController.OnPunch(PunchID.RIGHTJAB);
            }
            else
            {
